...
class AdminRepositoryImpl implements AdminRepository {
  ...
  Future<List<Map<String, dynamic>>> getMonthlyAttendanceReport(
    ...
    try {
      ...
      for (final teacherRecord in teachers) {
        final teacherId = teacherRecord.id;
        final teacherName = teacherRecord.getStringValue('name');
        final teacherNip = teacherRecord.getStringValue('nip');

        // Filter for this teacher
        final teacherAttendances =
            attendances
                .where((a) => a.getStringValue('teacher_id') == teacherId)
                .toList();

        int present = 0;
        int late = 0;
        int permit = 0; // Izin/Sakit/Cuti
        int alpha = 0; // Not recorded here directly usually, but logic depends on schedule

        // Count present/late from attendances
        for (final att in teacherAttendances) {
          final status = att.getStringValue('status');
          // If type is 'leave', it counts as permit
        ...
        // Also check raw leave requests to ensure coverage?
        // Actually, our LeaveRepository.approveLeave generates 'attendance' records with type='leave'.
        // So relying on 'attendances' collection is sufficient IF the approval process was followed.
        // For now, let's assume 'attendances' is the source of truth for stats.

        // Alpha calculation is tricky without generating daily expected records.
        // If we only have positive records, we can't count alpha unless we know how many days they SHOULD have been there.
        // For simplicity in this iteration, we will just sum the 'alpha' records if they exist (generated by a cron/admin job)
        // OR leave alpha as 0 if not tracked explicitly.
        // NOTE: Previous logic in 'AdminRepositoryImpl.getDailyAttendanceStats' calculated alpha as (Total - Present - Leave).
        // That works for a single day snapshot. For a month, it's harder without knowing total workdays.
